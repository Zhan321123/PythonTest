...
def genQpPlot():
    ...
    ax1.set_ylabel('流量 m^3/s',fontsize=20)
    ax1.set_title('Qp法计算各月生态流量', fontsize=22)
    ax1.grid()
    ax1.legend(fontsize=20)

    ax2.plot(y, qy90, label='90%保证率', marker='+')
    # ax2.plot(monthData.getYearSet(), qy95, label='Q year 95%', marker='+')
    ax2.set_xlabel('年份',fontsize=20)
    ax2.set_xticks(y, y)
    # 旋转45度
    ax2.set_xticklabels(y, rotation=45)
    ax2.set_ylabel('流量 m^3/s',fontsize=20)
    ax2.tick_params(axis='both', which='major', labelsize=22)
    ax2.set_title('Qp法计算各年生态流量', fontsize=22)
