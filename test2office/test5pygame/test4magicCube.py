...
class MagicCube:
    ...
    def __init__(self, screen, target_fps):
        ...
        self.mid_screen = numpy.array([int(self.width / 2), int(self.height / 2)], dtype=numpy.float32)
        ...
        self.disc = numpy.array([-1, -1, -1])
        self.disc_direction = 1
        self.disc_anim_ms = numpy.sqrt(self.size) * 200  # make anims slower with bigger cubes
        self.disc_anim_start = 0
        self.disc_anim_phase = 0
        self.disc_anim_cubies = numpy.zeros((0), dtype=numpy.int32)
        self.disc_anim_neighbors = numpy.zeros((0), dtype=numpy.int32)
        ...
        self.button_coords = numpy.zeros((0, 4), dtype=numpy.int16)
        self.button_types = []
        ...
        self.timer_names = []
        self.timers = numpy.zeros((1, 1), dtype=int)
        self.timer_frame = 0
        ...
        self.timers = numpy.zeros((len(self.timer_names), self.timer_avg_frames), dtype=int)
    ...
    def button_display(self, screen):
        ...
        mouse_pos = numpy.asarray(pygame.mouse.get_pos())
        buttons_over = numpy.nonzero(
        ...
        if numpy.shape(buttons_over)[1] == 1:
            ...
            if pygame.mouse.get_pressed()[0]:
                ...
                screen.blit(self.screen_button_selected, self.button_coords[button_nr, 0:2] + numpy.array([-10, -(self.button_y_space - 3)]))
                ...
                screen.blit(self.screen_button_mouseover, self.button_coords[button_nr, 0:2] + numpy.array([-10, -(self.button_y_space - 3)]))
    ...
    def rotate_disc(self, cube, time):
        ...
        if time - self.disc_anim_start < self.disc_anim_ms:
            angles = (self.disc >= 0) * (numpy.pi / 180) * (90 * self.disc_direction * (time - self.disc_anim_start) / self.disc_anim_ms)
        else:
            # animation finished, rotate full 90 degrees.
            angles = (self.disc >= 0) * (numpy.pi / 180) * 90 * self.disc_direction
        ...
        disc_nodes = numpy.repeat(self.disc_anim_cubies, 8) * 8 + numpy.resize(numpy.arange(8), numpy.size(self.disc_anim_cubies) * 8)
        matrix = self.rotate_matrix(angles)
        cube.cubie_rotated_disc_nodes[disc_nodes, :] = numpy.matmul(cube.cubie_rotated_disc_nodes[disc_nodes, :], matrix)
        ...
        if time - self.disc_anim_start >= self.disc_anim_ms:
            ...
            if x >= 0:
                cube.cube[x, :, :] = numpy.rot90(cube.cube[x, :, :], -self.disc_direction)
            elif y >= 0:
                cube.cube[:, y, :] = numpy.rot90(cube.cube[:, y, :], self.disc_direction)
            elif z >= 0:
                cube.cube[:, :, z] = numpy.rot90(cube.cube[:, :, z], -self.disc_direction)
            # rotate colors for these cubies
            surf_rot = cube.surface_rotation[numpy.sign(y) * 1 + numpy.sign(z) * 2, :]  # x = 0, y = 1, z = 2
            for cubie in self.disc_anim_cubies:
                cube.cubie_surf_color_nrs[cubie, surf_rot] = numpy.roll(cube.cubie_surf_color_nrs[cubie, surf_rot], self.disc_direction)
    ...
    def get_disc_neighbors(self, cube, disc):
        ...
        (x, y, z) = disc
        neighbors_1a = numpy.zeros((0), dtype=numpy.int32)
        neighbors_1b = numpy.zeros((0), dtype=numpy.int32)
        neighbors_2a = numpy.zeros((0), dtype=numpy.int32)
        neighbors_2b = numpy.zeros((0), dtype=numpy.int32)
        if x >= 0:
            cubies = cube.cube[x, :, :]
            if x > 0:
        ...
        return numpy.hstack((cubies.ravel(), neighbors_1a.ravel(), neighbors_1b.ravel(), neighbors_2a.ravel(), neighbors_2b.ravel()))
    ...
    def angle_add(self, cube, time, prev_time):
        ...
        unit_vector = numpy.array([0, 0, 0], dtype=numpy.float32)
        ...
        if keys[pygame.K_PAGEDOWN] or self.mouse_rotate == 'PAGE DOWN':
            unit_vector[2] -= 1
        if not numpy.all(numpy.equal(unit_vector, numpy.array([0, 0, 0]))):
            ...
            unit_vector /= numpy.sqrt(numpy.sum(unit_vector ** 2))  # scale to unit vector if multiple axes used
            matrix = self.rotate_matrix(cube.angles)
            (x, y, z) = numpy.matmul(unit_vector, matrix)
            angle = 1.0 * time_adjustment
            sa = numpy.sin(angle)
            ca = numpy.cos(angle)
            matrix_uv = numpy.array([[ca + x * y * (1 - ca)    , x * y * (1 - ca) - z * sa, x * z * (1 - ca) + y * sa],
            ...
            matrix_final = numpy.matmul(matrix, matrix_uv)
            # and the resulting angles can be calculated from that matrix.
            angle_x = numpy.arctan2(-matrix_final[1, 2], matrix_final[2, 2])
            angle_y = numpy.arctan2(matrix_final[0, 2], numpy.sqrt(1 - matrix_final[0, 2] ** 2))
            angle_z = numpy.arctan2(-matrix_final[0, 1], matrix_final[0, 0])
            cube.angles = numpy.array([angle_x, angle_y, angle_z])
            self.paused = True

        if not self.paused:
            # automatic rotation
            cube.angles += numpy.array([0.32, 0.37, 0.24]) * time_adjustment

    def rotate(self, cube):

        matrix = self.rotate_matrix(cube.angles)
        cube.cubie_rotated_nodes = numpy.matmul(cube.cubie_rotated_disc_nodes, matrix)
        cube.cube_corner_rotated_nodes = numpy.matmul(cube.cube_corner_nodes, matrix)

        size_adj = self.height * cube.cube_size_adj * cube.z_pos / ((2 + cube.distance) * cube.size)
        cube.cubie_trans_nodes = (cube.cubie_rotated_nodes[:, 0:2] / (cube.z_pos + cube.cubie_rotated_nodes[:, 2:3]) * size_adj + self.mid_screen).astype(numpy.int16)

        self.measure_time("rotate")

        cube.cubie_surface_angle_viewer = self.calculate_angle_viewer(cube, cube.cubie_rotated_nodes, cube.surf_nodes, numpy.array([0.0, 0.0, cube.z_pos]))
        cube.cube_surface_angle_viewer = self.calculate_angle_viewer(cube, cube.cube_corner_rotated_nodes, cube.surfaces, numpy.array([0.0, 0.0, cube.z_pos]))

        self.measure_time("calc visible")

    def rotate_matrix(self, angles):

        (sx, sy, sz) = numpy.sin(angles)
        (cx, cy, cz) = numpy.cos(angles)

        # build a matrix for X, Y, Z rotation (in that order, see Wikipedia: Euler angles).
        return numpy.array([[cy * cz               , -cy * sz              , sy],
    ...
    def add_labels(self, screen, cube):
        ...
        size_adj = self.height * cube.cube_size_adj * cube.z_pos / ((2 + cube.distance) * cube.size)
        trans_nodes = (cube.cube_corner_rotated_nodes[:, 0:2] / (cube.z_pos + cube.cube_corner_rotated_nodes[:, 2:3]) * size_adj + self.mid_screen).astype(numpy.int16)
        ...
        visible_nodes_count = numpy.bincount(visible_surf.ravel(), minlength=8)
        for i in range(3):
            node_use = numpy.sum(visible_nodes_count[cube.cube_corner_edges[i, :, :]], axis=1)
            node_use_select = numpy.max(node_use[node_use < 4])
            if node_use_select > 1:
                avg_coord = numpy.sum(trans_nodes[cube.cube_corner_edges[i, :, :], :], axis=(1, 2))
                edge_sort = numpy.argsort(avg_coord)[::-1]
                ...
                for cubie in range(cube.size):
                    ...
                    key_pic_coord = (key_pic_node[0:2] / (cube.z_pos + key_pic_node[2]) * size_adj + self.mid_screen - key_pic_size / 2).astype(numpy.int16)
    ...
    def draw(self, screen, cube):
        ...
        max_colors = numpy.max((cube.surf_color_nrs * (cube.cubie_surface_angle_viewer < 0)).reshape(cube.cubies_nr, 6), axis=1)
        # make sure all currently rotated cubies are included, even if they otherwise would not.
        if self.disc_anim_phase > 0 and numpy.size(self.disc_anim_cubies) > 0:
        ...
        cubies = numpy.arange(cube.cubies_nr)[max_colors > 0]
        # select surfaces to draw
        surfs = (numpy.repeat(cubies, 6) * 6 + numpy.resize(numpy.arange(6), (numpy.size(cubies) * 6)))
        ...
        if cube.size <= 20:
            ...
            if self.disc_anim_phase > 0 and numpy.size(self.disc_anim_cubies) > 0 and cube.size > 20:
                rotated_surfs = numpy.zeros(numpy.size(cube.surf_color_nrs), dtype=numpy.uint8)
                rotated_surfs[(numpy.repeat(self.disc_anim_neighbors, 6) * 6 + numpy.resize(numpy.arange(6), (numpy.size(self.disc_anim_neighbors) * 6)))] = 1
        ...
        surf_z = numpy.average(cube.cubie_rotated_nodes[cube.surf_nodes[surfs_visible, :], 2], axis=1)
        ...
        if self.disc_anim_phase == 0:
            ...
            if self.shuffle_count < 0:
                ...
                if undo:
                    ...
                    while direction == 0:
                        disc_axis = numpy.random.randint(0, 3)
                        disc_nr = numpy.random.randint(0, cube.size)
                        disc = ((numpy.arange(3) == disc_axis) * (disc_nr + 1) - 1).astype(numpy.int16)
                        direction = int(numpy.random.randint(0, 2) * 2 - 1)
                        if len(cube.moves) > 0:
                            if numpy.all(numpy.equal(disc, cube.moves[-1][0])) and direction == -cube.moves[-1][1]:
                                direction = 0
    ...
    def setup_buttons(self):
        ...
        for button in button_list:
            ...
            coords = numpy.asarray(rect) + numpy.array([0, 0, rect[0], rect[1]])  # convert rect to coordinates
            self.button_coords = numpy.vstack((self.button_coords, coords[None, :]))  # add button coords to button coordinate array
...
class Cube:
    ...
    def __init__(self, size, distance, screen_height, disc_keys_all):
        ...
        self.z_pos = size * 10
        self.cube_size_adj = 0.8 / numpy.sqrt(3)  # cube maximum size (percentage of screen y) adjusted for rotation
        self.cube_surface_line_width = int(max(1, screen_height / (self.size * 35)))
        self.cube = -numpy.ones((size, size, size), dtype=numpy.int32)
        ...
        self.surface_directions = numpy.average(self.nodes[self.surfaces], axis=1).astype(numpy.int32)
        # surface rotation defines the surfaces rotated for each (x, y, z) rotation, direction = 1
        self.surface_rotation = numpy.array([
            [0, 2, 1, 4],
            [3, 4, 5, 2],
            [3, 0, 5, 1]
            ], dtype=numpy.int32)
        self.cubie_nodes = numpy.zeros((8 * self.cubies_nr, 3), dtype=numpy.float32)
        # surf_nodes has the rotated node numbers for all surfaces
        self.surf_nodes = numpy.resize(self.surfaces, (self.cubies_nr * 6, 4)) + numpy.repeat(numpy.arange(self.cubies_nr) * 8, 6)[:, None]
        self.surf_colors = numpy.zeros((6 * self.cubies_nr, 3), numpy.uint8)
        self.surf_color_nrs = numpy.zeros((6 * self.cubies_nr), numpy.uint8)
        self.cubie_surf_color_nrs = numpy.zeros((self.cubies_nr, 6), numpy.uint8)

        self.moves = []
        self.angles = numpy.zeros((3), dtype=numpy.float32)
        ...
        for x in range(size):
            for y in range(size):
                for z in range(size):
                    ...
                    if x == 0:
                        colors.append(1)
                        directions.append(numpy.array([-1, 0, 0]))
                    elif x == size - 1:
                        colors.append(2)
                        directions.append(numpy.array([1, 0, 0]))
                    if y == 0:
                        colors.append(3)
                        directions.append(numpy.array([0, -1, 0]))
                    elif y == size - 1:
                        colors.append(4)
                        directions.append(numpy.array([0, 1, 0]))
                    if z == 0:
                        colors.append(5)
                        directions.append(numpy.array([0, 0, -1]))
                    elif z == size - 1:
                        colors.append(6)
                        directions.append(numpy.array([0, 0, 1]))
                    if len(colors) > 0:
                        self.cube[x, y, z] = cubie_nr
                        cubie_colors = numpy.zeros((6), dtype=numpy.uint8)
                        for i in range(len(directions)):
                            surf = (self.surface_directions == directions[i]).all(axis=1).nonzero()[0]
                            cubie_colors[surf] = colors[i]

                        self.cubie_nodes[8 * cubie_nr:8 * (cubie_nr + 1), :] = self.nodes + numpy.array([x - mid, y - mid, z - mid]) * (2 + distance)
        ...
        self.cubie_trans_nodes = numpy.zeros((8 * self.cubies_nr, 2), dtype=numpy.int16)
        self.cubie_surface_angle_viewer = numpy.zeros((6 * self.cubies_nr), dtype=numpy.float32)

        self.cube_corner_nodes = self.nodes * (1 + mid * (2 + distance))
        self.cube_corner_rotated_nodes = self.cube_corner_nodes.copy()
        self.cube_surface_angle_viewer = numpy.zeros((6), dtype=numpy.float32)
        self.cube_corner_edges = numpy.array([
        ...
        for keys in disc_keys_all:
            ...
            for m in range(mult):
                for key in keys:
                    ...
                    f_size = numpy.asarray(f_screen.get_size())
