...
class Life:
    def __init__(self, screen, target_fps):
        ...
        # self.rng = numpy.random.default_rng(self.random_seed)  # initiate with seed
        self.generation = 0
        self.life_amount = 0
        self.fade_cnt = 60
    ...
    def new_generation(self):
        ...
        #   C1. otherwise it is dead.
        self.life_array[1:w + 1, 1:h + 1][nb_array[:, :] == 3] = 1  # applying (A)   - (B) needs no action
        self.life_array[1:w + 1, 1:h + 1][(nb_array[:, :] < 3) | (nb_array[:, :] > 4)] = 0  # applying (C1)
